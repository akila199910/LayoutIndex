...
if (!function_exists('action_btns_po')) {
    function action_btns_po($action, $user, $edit_url, $can_delete, $read_url, $permission, $order)
    {
        ...
        if ($user->hasPermissionTo('Create_PurchaseOrder') && $order->status == 6 &&  $order->supplier_Info->deleted_at == null && $order->can_reorder == true) {
        ...
        $permissons = ['PO_Approval', 'PO_Hold', 'PO_Cancel', 'PO_Fullfillment', 'PO_Received', 'PO_Closed', 'Read_PurchaseOrder'];

        if ($user->hasAnyPermission($permissons)) {
            $action .= '<a class="dropdown-item" title="View Order" href="' . $read_url . '"><i class="fa-solid fa-eye m-r-5"></i> View Order</a>';
        }
...
if (!function_exists('action_btns_po_return')) {
    function action_btns_po_return($action, $user, $edit_url, $can_delete, $read_url, $permission,  $return, $view_url)
    {
        ...
        $permissons = ['PO_Return_Approval', 'PO_Return_Hold', 'PO_Return_Cancel', 'PO_Return_Fullfillment', 'PO_Return_Received', 'PO_Return_Closed','Read_PurchaseReturn'];

        if ($user->hasAnyPermission($permissons)) {
            $action .= '<a class="dropdown-item" title="View Return" href="' . $read_url . '"><i class="fa-solid fa-eye m-r-5"></i> View Return</a>';
        }
